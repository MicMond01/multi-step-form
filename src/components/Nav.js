  ...
  <div>
    <div className='sm:block  bg-desktopbg bg-center bg-cover bg-no-repeat w-[250px] h-full rounded-[12px] pt-8 pl-7 hidden'>  
      {navigation.map((navi, index) => (
        ...
        <Link to={`${navi.lead}`}>
          <div className={`${location.pathname === navi.lead ? " bg-Light text-Marine font-semibold": "bg-transparent"} text-white border-[1px] border-white py-1 px-2.5 rounded-[50%] mr-4 cursor-pointer `}>
            
              <p className={`w-[10px]  `}>{navi.id}</p>
          </div>
          </Link>
          <div className='leading-[15px]'>
            <div className=' text-[12px] text-Coolg font-medium'>{navi.step}</div>
            <div className='text-[14px] font-semibold text-Alabaster'>{navi.title}</div>
          </div>
        </div>
      ))}
      {/* <Link to='/Thanks'>
